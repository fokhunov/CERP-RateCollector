...
class ParserUZ(base.Parser):
    ...
    def parse_nbu(self):
        result = self.fetcher.fetch(
            "https://nbu.uz/en/exchange-rates/"
        )
        ...
        try:
            ...
            if tags:
                rates = {}
                for tag in tags:
                    tds = tag.findChildren('td')
                    if len(tds) < 1:
                        continue

                    if tds[1].text == "USD":
                        rates['usd_buy'] = rates['usd_sale'] = rate.from_string(tds[2].getText())
                        continue

                    if tds[1].text == "EUR":
                        rates['eur_buy'] = rates['eur_sale'] = rate.from_string(tds[2].getText())
                        continue

                    if tds[1].text == "RUB":
                        rates['rub_buy'] = rates['rub_sale'] = rate.from_string(tds[2].getText())
                        continue
    ...
    def parse_kdb(self):
        ...
        try:
            context = BeautifulSoup(
                result,
                'html.parser',
                parse_only=SoupStrainer('div', {'class': re.compile(r'exchange_widget_text')})
            )
            tags = context.find_all('tr')
            if tags:

                return {
                    'usd_buy': rate.from_string(tags[1].findChildren('td')[1].getText()),
                    'usd_sale': rate.from_string(tags[1].findChildren('td')[2].getText()),
                    'eur_buy': rate.from_string(tags[2].findChildren('td')[1].getText()),
                    'eur_sale': rate.from_string(tags[2].findChildren('td')[2].getText()),
    ...
    def parse_xb(self):
        result = self.fetcher.fetch(
            "http://www.xb.uz/ru/interactive/rates"
        )
            ...
            if tags:
                ...
                for tag in tags:
                    if re.search(r'USD', tag.text):
                        rates['usd_buy'] = rate.from_string(tag.findChildren('td')[4].getText())
                        rates['usd_sale'] = rate.from_string(tag.findChildren('td')[5].getText())
                        continue

                    if re.search(r'EUR', tag.text):
                        rates['eur_buy'] = rate.from_string(tag.findChildren('td')[4].getText())
                        rates['eur_sale'] = rate.from_string(tag.findChildren('td')[5].getText())
                        continue

                    if re.search(r'RUB', tag.text):
                        rates['rub_buy'] = rate.from_string(tag.findChildren('td')[4].getText())
                        rates['rub_sale'] = rate.from_string(tag.findChildren('td')[5].getText())
                        continue
    ...
    def parse_aloqabank(self):
        result = self.fetcher.fetch(
            "http://www.aloqabank.uz/ru/page/interactive/rates"
        )
    ...
    def parse_trustbank(self):
        result = self.fetcher.fetch(
            "http://trustbank.uz/ru/services/exchange-rates/"
        )
